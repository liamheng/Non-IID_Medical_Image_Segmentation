...
class UnetCombine2LayerGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, num_downs, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, last_layer='Tanh'):
        ...
        unet_block1 = UnetSkipConnectionBlock(output_nc, ngf, input_nc=input_nc, outermost=True, norm_layer=norm_layer, last_layer=last_layer)  # add the outermost layer
    ...
    def forward(self, x):
        ...
        # upsample
        h_u8 = self.h_up8(d8)
        h_u7 = self.h_up7(torch.cat([h_u8, d7], 1))
        h_u6 = self.h_up6(torch.cat([h_u7, d6], 1))
        h_u5 = self.h_up5(torch.cat([h_u6, d5], 1))
        h_u4 = self.h_up4(torch.cat([h_u5, d4], 1))
        h_u3 = self.h_up3(torch.cat([h_u4, d3], 1))
        h_u2 = self.h_up2(torch.cat([h_u3, d2], 1))
        h_u1 = self.h_up1(torch.cat([h_u2, d1], 1))
        ...
        # TODO: 完善
        u8 = self.up8(d8)
        u7 = self.up7(torch.cat([h_u8, u8], 1))
        u6 = self.up6(torch.cat([h_u7, u7], 1))
        u5 = self.up5(torch.cat([h_u6, u6], 1))
        u4 = self.up4(torch.cat([h_u5, u5], 1))
        u3 = self.up3(torch.cat([h_u4, u4], 1))
        u2 = self.up2(torch.cat([h_u3, u3], 1))
        u1 = self.up1(torch.cat([h_u2, u2], 1))
        # return u1
        # print(h_u1.min(), h_u1.max(), '1111111111111111111111111111111111111111')
        # print(u1.min(), h_u1.max(), '1111111111111111111111111111111111111111')
