...
if __name__ == '__main__':
    ...
    for name in tqdm(os.listdir(os.path.join(opt.checkpoints_dir, opt.name))):
        if not name.endswith('.pth'):
            continue
        model.setup(opt, load_suffix=name[:name.find('_net')], print_network=not opt.verbose)
        if opt.eval:
            model.eval()
    ...
    with open(os.path.join(opt.checkpoints_dir, opt.name, 'test_analysis.csv'), 'w') as result_file:
    ...
    if opt.result_output_file_path is not None:
        with open(opt.result_output_file_path, 'a') as result_file:
            result_file.write(metrics_result_list[0][0])
